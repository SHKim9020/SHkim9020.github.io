(() => {
  ...
  function initResponsiveViewport(){const desktop=()=>window.innerWidth>900;let wasDesktop=desktop();const resetDesktopScroll=()=>{const isDesktop=desktop();if(isDesktop&&(window.scrollY||document.documentElement.scrollTop))window.scrollTo(0,0);if(isDesktop!==wasDesktop){wasDesktop=isDesktop;requestAnimationFrame(()=>{window.scrollTo(0,0);Blockly.svgResize(workspace)})}};window.addEventListener("resize",resetDesktopScroll,{passive:true});if(wasDesktop)requestAnimationFrame(()=>window.scrollTo(0,0))}
