...
void applyConfig(JsonObjectConst c){JsonObjectConst p=c["pins"];config.in1=p["in1"]|12;config.in2=p["in2"]|13;config.in3=p["in3"]|14;config.in4=p["in4"]|15;config.invertLeft=c["invertLeft"]|false;config.invertRight=c["invertRight"]|true;JsonObjectConst cam=c["camera"];config.frameSize=(const char*)(cam["frameSize"]|"VGA");config.quality=cam["quality"]|12;config.flip=cam["flip"]|false;setupMotorOutputs();stopCar();applyCameraSettings();}
...
void setupWebRoutes(){
  ...
  webServer.on("/api/status",HTTP_GET,[](){JsonDocument d;d["camera"]=cameraReady;d["cameraError"]=cameraError;d["psram"]=psramFound();d["freeHeap"]=ESP.getFreeHeap();d["wifi"]=wifiName();d["ip"]=WiFi.softAPIP().toString();String out;serializeJson(d,out);webServer.send(200,"application/json",out);});
...
void setup(){
  ...
  LittleFS.begin(true);setupMotorOutputs();setupCamera();loadProgram();startWifi();if(activeDocument.size())startProgram();emit("ready",String("OneMaker ESP32-CAM RC Runtime 0.1.1 / camera ")+(cameraReady?"OK":cameraError));
}
void loop(){
