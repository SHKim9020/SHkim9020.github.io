...
// OneMaker ESP32-CAM RC Runtime 0.1.15 — face detection and BLE program transfer
...
String lastFaceSide="center";
bool pendingHandlerUseUiSpeed=false;
bool centerFaceTurnLeft=true;
...
function send(d){return call('/api/drive?dir='+d+'&left='+L.value+'&right='+R.value+'&blocks='+(blocks?1:0))}
...
bool waitMotion(unsigned long duration){unsigned long start=millis();while(millis()-start<duration){if(programTaskStop){stopCar();return false;}delay(10);}return true;}
bool executeSteps(JsonArrayConst steps){
  for(JsonObjectConst s:steps){if(programTaskStop)return false;String op=s["op"]|"";
    if(op=="drive"){int speed=(int)eval(s["speed"]);drive(s["dir"]|"stop",remoteUiSpeedActive?remoteUiLeftSpeed:speed,remoteUiSpeedActive?remoteUiRightSpeed:speed);}
      ...
      int speed=constrain((int)eval(s["speed"]),0,255);stopCar();if(!waitMotion(160))return false;
      drive("backward",speed,speed);if(!waitMotion(550))return false;
      String turn=lastFaceSide=="left"?"right":lastFaceSide=="right"?"left":(centerFaceTurnLeft?"left":"right");
      if(lastFaceSide=="center")centerFaceTurnLeft=!centerFaceTurnLeft;drive(turn,speed,speed);if(!waitMotion(650))return false;stopCar();
    }
...
class RcBleServerCallbacks:public NimBLEServerCallbacks{
...
class RcBleUploadCallbacks:public NimBLECharacteristicCallbacks{
...
void setupBluetooth(){
  NimBLEDevice::init((String("OneMaker-RC-BLE-")+twoDigits()).c_str());NimBLEDevice::setMTU(185);NimBLEServer *server=NimBLEDevice::createServer();server->setCallbacks(new RcBleServerCallbacks());NimBLEService *service=server->createService(BLE_SERVICE_UUID);
  bleTxCharacteristic=service->createCharacteristic(BLE_TX_UUID,NIMBLE_PROPERTY::NOTIFY);NimBLECharacteristic *rx=service->createCharacteristic(BLE_RX_UUID,NIMBLE_PROPERTY::WRITE);rx->setCallbacks(new RcBleUploadCallbacks());service->start();
  NimBLEAdvertising *advertising=NimBLEDevice::getAdvertising();advertising->addServiceUUID(BLE_SERVICE_UUID);advertising->setScanResponse(true);NimBLEDevice::startAdvertising();emit("ble",String("OneMaker-RC-BLE-")+twoDigits());
}
...
void handleSerialLine(const String &line){
  ...
  if(cmd=="hello"){stopProgram();stopCar();JsonDocument info;info["type"]="info";info["runtime"]="0.1.15";info["board"]="ESP32-CAM AI Thinker";info["wifi"]=wifiName();serializeJson(info,Serial);Serial.println();return;}
...
void setup(){
  ...
  LittleFS.begin(true);setupCamera();loadProgram();setupBluetooth();startWifi();bool usbCommandWaiting=false;unsigned long safetyStart=millis();while(millis()-safetyStart<5000){if(Serial.available()){usbCommandWaiting=true;break;}delay(10);}if(activeDocument.size()&&!usbCommandWaiting)startProgram();else stopCar();emit("ready",String("OneMaker ESP32-CAM RC Runtime 0.1.15 / camera ")+(cameraReady?"OK":cameraError));
}
void loop(){
