...
// OneMaker ESP32-CAM RC Runtime 0.1.0 — AI Thinker + L9110S/MX1508
...
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#07121f;color:#e9f6ff;font-family:system-ui,sans-serif;touch-action:manipulation}main{max-width:680px;margin:auto;padding:12px}.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:9px}.top h1{font-size:18px;margin:0}.top span{font-size:10px;color:#75dce8}.video{position:relative;background:#02070d;border:1px solid #28475b;border-radius:17px;overflow:hidden;aspect-ratio:4/3;display:grid;place-items:center}.video img{width:100%;height:100%;object-fit:contain}.hud{position:absolute;inset:10px 12px auto;display:flex;justify-content:space-between;font:700 10px monospace;text-shadow:0 1px 4px #000}.hud i{color:#ff6370}.controls{margin-top:10px;background:#101f2d;border:1px solid #284052;border-radius:17px;padding:12px}.speed{display:grid;grid-template-columns:42px 1fr 35px;gap:8px;align-items:center;font-size:11px}.speed input{width:100%;accent-color:#22c5dc}.speed output{font-weight:900;color:#5fe5f2}.pad{display:grid;grid-template:64px 64px 64px/repeat(3,78px);gap:7px;justify-content:center;margin:9px auto}button{border:0;border-radius:15px;background:#21384a;color:#e9f6ff;font-size:25px;font-weight:900;touch-action:none;box-shadow:inset 0 -3px #152938}.up{grid-column:2}.left{grid-row:2;grid-column:1}.stop{grid-row:2;grid-column:2;background:#5b2630;color:#ffced2}.right{grid-row:2;grid-column:3}.down{grid-row:3;grid-column:2}button small{display:block;font-size:9px}.on{outline:3px solid #4fe5ef;background:#205568}.tools{display:flex;justify-content:center;gap:8px}.tools button{font-size:11px;padding:9px 12px;background:#1a3041}.status{text-align:center;font-size:10px;color:#8ba5b4;margin:7px}.landscape{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:10px}@media(max-width:620px){.landscape{display:block}}@media(orientation:landscape) and (max-height:560px){main{max-width:100%;padding:6px}.landscape{display:grid;grid-template-columns:minmax(0,1fr) 290px}.video{max-height:calc(100vh - 48px)}.top{margin-bottom:4px}.controls{margin-top:0}.pad{grid-template:55px 55px 55px/repeat(3,74px)}}
</style></head><body><main><div class="top"><h1>📹 OneMaker 영상탐사 RC카</h1><span id="net">● Wi‑Fi 연결됨</span></div><div class="landscape"><div class="video"><img id="stream" alt="ESP32-CAM 실시간 영상"><div class="hud"><i>● LIVE</i><b id="motion">정지</b></div></div><section class="controls"><label class="speed">왼쪽<input id="leftSpeed" type="range" min="0" max="255" value="150"><output id="lv">150</output></label><label class="speed">오른쪽<input id="rightSpeed" type="range" min="0" max="255" value="150"><output id="rv">150</output></label><div class="pad"><button class="up" data-dir="forward">▲<small>전진</small></button><button class="left" data-dir="left">◀<small>좌회전</small></button><button class="stop" data-dir="stop">■<small>정지</small></button><button class="right" data-dir="right">▶<small>우회전</small></button><button class="down" data-dir="backward">▼<small>후진</small></button></div><div class="tools"><button id="flash">💡 조명</button><button id="quality">📷 화질</button><button id="flip">🔄 화면회전</button></div><p class="status">버튼을 누르는 동안만 움직이며 연결이 끊기면 자동 정지합니다.</p></section></div></main><script>
const $=s=>document.querySelector(s),L=$('#leftSpeed'),R=$('#rightSpeed');$('#stream').src='http://'+location.hostname+':81/stream';L.oninput=()=>$('#lv').value=L.value;R.oninput=()=>$('#rv').value=R.value;let held='stop',timer=null,light=false,flipped=false,size='VGA';const labels={forward:'전진 중',backward:'후진 중',left:'좌회전 중',right:'우회전 중',stop:'정지'};
...
document.querySelectorAll('[data-dir]').forEach(b=>{b.onpointerdown=e=>{e.preventDefault();b.setPointerCapture?.(e.pointerId);press(b.dataset.dir)};if(b.dataset.dir!=='stop'){b.onpointerup=()=>release();b.onpointercancel=()=>release();b.onpointerleave=e=>{if(e.buttons)release()}}});
$('#flash').onclick=()=>{light=!light;call('/api/flash?on='+(light?1:0))};$('#flip').onclick=()=>{flipped=!flipped;call('/api/flip?on='+(flipped?1:0))};$('#quality').onclick=()=>{size=size==='QVGA'?'VGA':size==='VGA'?'SVGA':'QVGA';$('#quality').textContent='📷 '+size;call('/api/frame?size='+size)};addEventListener('pagehide',()=>navigator.sendBeacon('/api/stop'));document.addEventListener('visibilitychange',()=>{if(document.hidden)release(true)});
</script></body></html>
)HTML";
...
void writeMotor(int pinA,int pinB,int value){
  value=constrain(value,-255,255);
  if(value>0){analogWrite(pinA,value);analogWrite(pinB,0);}
  else if(value<0){analogWrite(pinA,0);analogWrite(pinB,-value);}
  else{analogWrite(pinA,0);analogWrite(pinB,0);}
}
void setMotors(int left,int right){if(config.invertLeft)left=-left;if(config.invertRight)right=-right;writeMotor(config.in1,config.in2,left);writeMotor(config.in3,config.in4,right);}
...
framesize_t parseFrameSize(const String &value){if(value=="QVGA")return FRAMESIZE_QVGA;if(value=="SVGA")return FRAMESIZE_SVGA;return FRAMESIZE_VGA;}
...
bool setupCamera(){
  camera_config_t c={};c.ledc_channel=LEDC_CHANNEL_0;c.ledc_timer=LEDC_TIMER_0;c.pin_d0=Y2_GPIO_NUM;c.pin_d1=Y3_GPIO_NUM;c.pin_d2=Y4_GPIO_NUM;c.pin_d3=Y5_GPIO_NUM;c.pin_d4=Y6_GPIO_NUM;c.pin_d5=Y7_GPIO_NUM;c.pin_d6=Y8_GPIO_NUM;c.pin_d7=Y9_GPIO_NUM;c.pin_xclk=XCLK_GPIO_NUM;c.pin_pclk=PCLK_GPIO_NUM;c.pin_vsync=VSYNC_GPIO_NUM;c.pin_href=HREF_GPIO_NUM;c.pin_sccb_sda=SIOD_GPIO_NUM;c.pin_sccb_scl=SIOC_GPIO_NUM;c.pin_pwdn=PWDN_GPIO_NUM;c.pin_reset=RESET_GPIO_NUM;c.xclk_freq_hz=20000000;c.pixel_format=PIXFORMAT_JPEG;c.frame_size=psramFound()?FRAMESIZE_VGA:FRAMESIZE_QVGA;c.jpeg_quality=12;c.fb_count=psramFound()?2:1;c.grab_mode=CAMERA_GRAB_LATEST;c.fb_location=psramFound()?CAMERA_FB_IN_PSRAM:CAMERA_FB_IN_DRAM;
  esp_err_t err=esp_camera_init(&c);if(err!=ESP_OK){emit("error",String("camera 0x")+String(err,HEX));return false;}applyCameraSettings();return true;
}
...
bool executeSteps(JsonArrayConst steps){
  for(JsonObjectConst s:steps){if(programTaskStop)return false;String op=s["op"]|"";
    ...
    else if(op=="cameraFrame"){config.frameSize=(const char*)(s["size"]|"VGA");applyCameraSettings();}
...
void applyConfig(JsonObjectConst c){JsonObjectConst p=c["pins"];config.in1=p["in1"]|12;config.in2=p["in2"]|13;config.in3=p["in3"]|14;config.in4=p["in4"]|15;config.invertLeft=c["invertLeft"]|false;config.invertRight=c["invertRight"]|true;JsonObjectConst cam=c["camera"];config.frameSize=(const char*)(cam["frameSize"]|"VGA");config.quality=cam["quality"]|12;config.flip=cam["flip"]|false;for(int pin:{config.in1,config.in2,config.in3,config.in4})pinMode(pin,OUTPUT);stopCar();applyCameraSettings();}
...
void startWifi(){WiFi.mode(WIFI_AP);WiFi.softAP(wifiName().c_str(),WIFI_PASSWORD);setupWebRoutes();startStreamServer();emit("wifi",wifiName()+" / http://192.168.4.1");}

void handleSerialLine(const String &line){
  JsonDocument d;DeserializationError e=deserializeJson(d,line);if(e){emit("error","JSON command");return;}String cmd=d["cmd"]|"";
  if(cmd=="hello"){JsonDocument info;info["type"]="info";info["runtime"]="0.1.0";info["board"]="ESP32-CAM AI Thinker";info["wifi"]=wifiName();serializeJson(info,Serial);Serial.println();return;}
  if(cmd=="stop"){stopProgram();ack("stopped");return;}
  if(cmd=="drive"){drive(d["dir"]|"stop",d["speed"]|150,d["speed"]|150);ack();return;}
  if(cmd=="flash"){digitalWrite(FLASH_LED,(bool)(d["on"]|false));ack();return;}
  if(cmd=="cameraFrame"){config.frameSize=(const char*)(d["size"]|"VGA");applyCameraSettings();ack();return;}
...
void setup(){
  Serial.begin(115200);delay(250);pinMode(FLASH_LED,OUTPUT);digitalWrite(FLASH_LED,LOW);Preferences p;p.begin("onemaker-rc",true);carNumber=p.getUChar("number",1);p.end();if(carNumber<1||carNumber>16)carNumber=1;
  LittleFS.begin(true);for(int pin:{12,13,14,15}){pinMode(pin,OUTPUT);analogWrite(pin,0);}setupCamera();loadProgram();startWifi();if(activeDocument.size())startProgram();emit("ready","OneMaker ESP32-CAM RC Runtime 0.1.0");
}
void loop(){
