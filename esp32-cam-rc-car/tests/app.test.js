...
for(const feature of ['0.1.1-s3','recoverCamera','cameraFrameFailures','first frame timeout'])assert(ino.includes(feature),`missing ESP32-S3 camera recovery feature ${feature}`);
const remote=fs.readFileSync(__dirname+'/../firmware/rc_cam_runtime/remote_page_v012.h','utf8');
for(const feature of ['connectStream','영상 재연결 중'])assert(remote.includes(feature),`missing camera stream retry feature ${feature}`);
